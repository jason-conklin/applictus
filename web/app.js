...
function isPaidPlanSource(source = sessionUser) {
  if (!source) {
    return false;
  }
  ...
  const activeJobSearchPlan =
    Boolean(source.job_search_plan_active) ||
    (
...
function getForwardingFilterHealthMeta(source = {}) {
  const status = String(source.forwarding_filter_status || '').trim().toLowerCase();
  const serverLabel = String(source.forwarding_filter_status_label || '').trim();
  const serverDetail = String(source.forwarding_filter_status_detail || '').trim();
  if (status === 'high_forwarding_volume') {
    return {
      status,
      tone: 'strong',
      label: serverLabel || 'High forwarding volume',
      detail: serverDetail || 'Forwarding volume is high. Tighten your Gmail filter so only job-related emails are forwarded.'
    };
  }
  if (status === 'filter_review_recommended') {
    return {
      status,
      tone: 'soft',
      label: 'Review filter',
  ...
  return {
    status: 'healthy',
    tone: 'healthy',
    label: serverLabel || 'Smart filter healthy',
    detail: serverDetail || 'Forwarding looks focused on job-related emails.'
  };
}
...
function renderPlanUsage(user = sessionUser) {
  ...
  const hasActiveJobSearchPlan =
    ...
    Number.isFinite(new Date(planExpiresAt).getTime()) &&
    new Date(planExpiresAt).getTime() > Date.now();
  const isPaidPlan = isPaidPlanSource(source);
  const limit = isPaidPlan ? null : (planUsage?.limit || getPlanLimitForUser(source));
  ...
  const inboundLimit = Number.isFinite(inboundLimitRaw) && inboundLimitRaw > 0
    ...
    : (isPaidPlan ? INBOUND_PLAN_LIMITS.pro : INBOUND_PLAN_LIMITS.free);
  ...
  const inboundWarningText =
    isPaidPlan && inboundWarningLevel !== 'none'
      ? forwardingHealth.detail
  ...
  const markerRow = accountPlanMarker?.closest('.plan-progress-marker-row');
  const scaleRow = accountPlanScaleStart?.closest('.plan-progress-scale-row');
...
const OUTLOOK_FORWARDING_HELP = [
  ...
  'Add a rule so only job-related messages forward to Applictus.'
].join('\n');
...
function getInboundSetupSubstepIndex(setupContext, phase) {
  if (!setupContext || !setupContext.substepByPhase) {
    return 0;
  }
  return Math.max(0, Number(setupContext.substepByPhase[phase] || 0));
}

function setInboundSetupSubstepIndex(setupContext, phase, nextIndex, maxIndex) {
  if (!setupContext || !setupContext.substepByPhase) {
    return;
  }
  const safeMax = Math.max(0, Number(maxIndex || 0));
  const safeNext = Math.max(0, Math.min(safeMax, Number(nextIndex) || 0));
  ...
  return {
    ...
    appendExtras: (target) => {
      ...
      completionNote.textContent =
        'This verifies the inbox address. Gmail may still ask you to click its confirmation link before filters can forward.';
  ...
  if (phase === 0) {
      ...
      substeps: [
        ...
        },
        {
          ...
          description: 'Paste the address above and continue. Gmail sends one confirmation email.',
          mediaFactory: () =>
          ...
          buildForwardingAnimatedTutorial({
            ...
            frames: [
              ...
              },
              {
                ...
                caption: 'Select your Applictus inbox address.'
              }
            ]
          }),
        appendExtras: (target) => {
...
function buildInboundSetupSecondaryHelp() {
  ...
  secondary.appendChild(outlookPanel.details);
...
function buildInboundSetupStep(phase, setupContext) {
  ...
  if (activeSubstep) {
    ...
    if (typeof activeSubstep.mediaFactory === 'function') {
      ...
      if (mediaNode) {
        const mediaWrap = document.createElement('div');
        mediaWrap.className = 'forwarding-substep-media';
        mediaWrap.appendChild(mediaNode);
        card.appendChild(mediaWrap);
      }
    }
    ...
    if (typeof activeSubstep.appendActions === 'function') {
      activeSubstep.appendActions(actions);
    }
    if (actions.children.length) {
      card.appendChild(actions);
    }

    if (typeof activeSubstep.appendExtras === 'function') {
      activeSubstep.appendExtras(card);
    }
...
function openInboundSetupModal({ startStep = 0 } = {}) {
  ...
  backBtn.addEventListener('click', () => {
    const currentConfig = getInboundSetupPhaseConfig(currentStep, setupContext);
    const currentCount = Math.max(1, currentConfig.substeps?.length || 1);
    const currentIndex = Math.max(0, Math.min(currentCount - 1, getInboundSetupSubstepIndex(setupContext, currentStep)));
    if (currentStep === 0 && currentIndex === 0) {
      closeModal('cancel');
      return;
    }

    if (currentIndex > 0) {
      setInboundSetupSubstepIndex(setupContext, currentStep, currentIndex - 1, currentCount - 1);
      render();
      return;
    }

    currentStep = Math.max(0, currentStep - 1);
    const previousConfig = getInboundSetupPhaseConfig(currentStep, setupContext);
    const previousMax = Math.max(0, (previousConfig.substeps?.length || 1) - 1);
    setInboundSetupSubstepIndex(setupContext, currentStep, previousMax, previousMax);
    render();
  });
  ...
  nextBtn.addEventListener('click', () => {
    ...
    currentStep = Math.min(1, currentStep + 1);
    const nextConfig = getInboundSetupPhaseConfig(currentStep, setupContext);
    const nextMax = Math.max(0, (nextConfig.substeps?.length || 1) - 1);
    setInboundSetupSubstepIndex(setupContext, currentStep, getInboundSetupSubstepIndex(setupContext, currentStep), nextMax);
    render();
  });
