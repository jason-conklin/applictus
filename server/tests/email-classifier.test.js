...
test('classifyEmail detects direct recruiter post-interview rejection from company domain', () => {
  ...
  assert.equal(relevance.isRelevant, true);
  assert.ok(relevance.matchedKeywords.includes('push_forward_other_candidates'));

  const result = classifyEmail({ subject, sender, body });
  assert.equal(result.isJobRelated, true);
  assert.equal(result.detectedType, 'rejection');
  assert.equal(Boolean(result.actionNeeded), false);
  assert.ok(result.confidenceScore >= 0.95);
  assert.ok(result.debug?.rejectionMatches.includes('push forward with other candidates'));
});

test('classifyEmail detects direct recruiter rejection wording variants', () => {
  const cases = [
    'We’ve decided to move forward with other candidates for this position.',
    'We are not moving forward at this time after reviewing candidates for the role.',
    'After your interview, we have decided not to proceed with your candidacy.',
    'Thank you for interviewing with our team. This was a tough decision, but we will proceed with other candidates.'
  ];

  for (const body of cases) {
    const result = classifyEmail({
      subject: 'Acme - Account Manager',
      sender: 'Recruiter <recruiter@acme.com>',
      body
    });
    assert.equal(result.isJobRelated, true, body);
    assert.equal(result.detectedType, 'rejection', body);
    assert.ok(result.confidenceScore >= 0.9, body);
  }
});
...
test('classifyEmail ignores non-job emails with ambiguous rejection words', () => {
  const newsletter = classifyEmail({
    subject: 'Weekly newsletter',
    sender: 'updates@example.com',
    body: 'Other candidates for your reading list are below. Unsubscribe anytime.'
  });
  assert.equal(newsletter.isJobRelated, false);

  const sales = classifyEmail({
    subject: 'Platform positioning',
    sender: 'sales@example.com',
    body: 'Our team can position your product better for the next budget cycle.'
  });
  assert.equal(sales.isJobRelated, false);

  const productUpdate = classifyEmail({
    subject: 'Product update',
    sender: 'updates@example.com',
    body: 'Unfortunately, this release was delayed. Our team will share a better match for your workflow soon.'
  });
  assert.equal(productUpdate.isJobRelated, false);
});
