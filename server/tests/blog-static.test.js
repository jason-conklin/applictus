...
const articles = [
  [
    ...
    'Best Free Job Application Trackers in 2026 | Applictus',
    '/applictus-blog-image8.png',
...
test('blog hub and articles are static, linked, and SEO-ready', () => {
  ...
  assert.match(hub, /href="\/blog\/best-free-job-application-trackers-2026"/);
  assert.match(hub, /Best Free Job Application Trackers in 2026/);
  ...
  const freeTrackerArticle = readProjectFile(
    'web/blog/best-free-job-application-trackers-2026/index.html'
  );
  assert.match(
    freeTrackerArticle,
    /<meta name="description" content="Compare the best free job application trackers in 2026 and learn what features matter most when organizing applications, interviews, offers, and rejections\."/
  );
  assert.match(freeTrackerArticle, /<span>Comparison<\/span>/);
  assert.match(freeTrackerArticle, /<span>7 min read<\/span>/);
  assert.match(freeTrackerArticle, /Why job seekers need a tracker/);
  assert.match(freeTrackerArticle, /What makes a good free job application tracker/);
  assert.match(freeTrackerArticle, /Spreadsheet trackers/);
  assert.match(freeTrackerArticle, /Notion-based trackers/);
  assert.match(freeTrackerArticle, /Dedicated application tracking tools/);
  assert.match(freeTrackerArticle, /Features to look for in 2026/);
  assert.match(freeTrackerArticle, /Choosing the right tracker for your workflow/);
  assert.match(freeTrackerArticle, /Applictus is one option in this category/);
  assert.match(freeTrackerArticle, /\/applictus-blog-image8\.png/);
  assert.doesNotMatch(freeTrackerArticle, /<h1>Free Job Application Tracker<\/h1>/);
